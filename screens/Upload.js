  ...
  _handleImagePicked = async pickerResult => {
    try {
      ...
      // if (!pickerResult.cancelled) {
        ...
        this.uploadImageAsync(pickerResult);
...
uploadImageAsync = async(uri)=> {
  ...
  const ref = 
    storage
    .ref()
    .child('images/'+that.state.bookId+'/'+that.uniqueId());
  const snapshot = await ref.put(blob)

  // var uploadTask = storageRef.child('images/user1234/file.txt').put(file);
  // .on('state_changed', function(data){
  //   var progress =((data.bytesTransferred / data.totalBytes)*100).toFixed(0);
  //   console.log('upload is '+ progress + '% complete');
  // })
  // const uploadTask = await ref.put(blob).on('state_changed', function(data){
  //   var progress =((data.bytesTransferred / data.totalBytes)*100).toFixed(0);
  //   console.log('upload is '+ progress + '% complete');
  //   that.setState({
  //     progress: progress
  ...
  //   }, function(){// upload is complete
  //     that.setState({progress: 100});
  //     uploadTask.data.ref.getDownloadURL().then(function(downloadURL){
  //       console.log(downloadURL)
  //       // that.processData(downloadURL);
  //     })
  //   })
